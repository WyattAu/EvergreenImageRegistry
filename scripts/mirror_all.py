    ...
    for img_dir in sorted(IMAGES_DIR.iterdir()):
        ...
        for line in text.splitlines():
            ...
            if not s.startswith("FROM "):
                continue
            if any(
                x in s for x in ["health-shim", "evergreenshim", "scratch", "mirror-"]
            ):
                continue
            parts = s.split()
            if len(parts) < 2:
...
def update_dockerfile(dockerfile, upstream, mirror_ref):
    text = dockerfile.read_text()
    pattern = re.compile(
        rf"(FROM\s+){re.escape(upstream)}(\s+AS\s+|\s*$|\s+--platform=)", re.MULTILINE
    )
...
def main():
    ...
    for i, (upstream, dfs) in enumerate(filtered.items()):
        ...
        # Cleanup
        subprocess.run(
            ["docker", "rmi", upstream, mirror_ref], capture_output=True, timeout=60
        )
