...
This is **shell form** — Docker wraps it as `/bin/sh -c "nginx -v"`. Since `FROM scratch` has no `/bin/sh`, this
**always fails** with:
...
Additionally, even if we switched to exec form, `CMD nginx -v` would be interpreted by the shell as running `nginx` with
argument `-v`, but since ENTRYPOINT is `["/nginx"]`, the HEALTHCHECK CMD is **appended to ENTRYPOINT**, resulting in
`/nginx nginx -v` — which is wrong.
...
**Pattern for HTTP health checks (requires embedding a health checker):** For images that serve HTTP but have no
curl/wget, we need to either:
...
**Decision:** For Phase 0, use binary version checks. HTTP health checks will be added in Phase 2 by embedding a minimal
static `wget` binary.
...
In shell form, this runs `python` with arguments `curl -s localhost:8080/api/status`. This is wrong — it should run
`curl` directly.
...
Of the 87 debian-slim images, some CAN be converted to multi-stage (download binary → copy to scratch) and some CANNOT
(they genuinely need the OS runtime).
...
GitHub Actions has a 256-job limit per workflow run. With 223 images, the matrix is near this limit. Additionally, the
`discover` step output may exceed GHA's output size limit.
...
1. `|| ((failed++))` fires: `failed` becomes 1, but `((failed++))` evaluates `failed` (which was 0), returns exit code 1
   (falsy in bash arithmetic)
