...
**Problem:** No automated CIS Docker Benchmark scanning exists. Manual compliance checks are error-prone and non-repeatable.
...
**Exclusions:** Document justified exceptions in `cis_config.yaml` with rationale (e.g., "scratch images cannot contain /etc/passwd modifications").
...
**Problem:** DISA STIG compliance requires manual evaluation against the Docker/Container STIG (currently SRG-APP-000XXX). No automated scanning exists.
...
**Problem:** Military environments have no external network access. All images, tools, and metadata must be transferable via physical media.
