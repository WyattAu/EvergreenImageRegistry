...
# =========================================================================
def go_binary(
    name, version, vendor, url, binary_name, port, healthcheck_cmd=None, source_url=None
):
...
def python_pip(
    ...
    hc_cmd = (
        f"CMD curl -sf http://localhost:{port}{hc}"
        if hc
        else f'CMD ["{ep}", "--version"]'
    )
...
""",
    ...
    url = (
        f"https://pypi.org/pypi/{packages[0]}/{version}/json"
...
def python_git(
    ...
    pip_extra = (
        f"&& pip3 install --no-cache-dir --break-system-packages {extra_pip} 2>/dev/null || true"
        if extra_pip
        else ""
    )
    write_dockerfile(
...
""",
    ...
    url = (
        source_url
        if source_url
        else f"https://github.com/{vendor}/{name}/releases/download/v{version}"
    )
    write_checksums(name, version, url)
...
def apt_get(
    ...
    port_line = f"\nEXPOSE {port}" if port else ""
    ep = (
        entrypoint or packages[0] if isinstance(packages, list) else packages.split()[0]
    )
    user = user or "app"
    hc = healthcheck_url or ""
    hc_line = ""
...
""",
    ...
    write_checksums(
        name,
        version,
        f"https://packages.debian.org/bookworm/{packages[0] if isinstance(packages, list) else packages.split()[0]}",
    )
...
# =========================================================================
def placeholder(
    name, version, vendor, note="Placeholder - no upstream binary available"
):
    write_dockerfile(
        name,
        f"""# =============================================================================
...
python_pip(
    ...
    "1.106.0",
    "Immich",
    ["onnxruntime", "fastapi", "uvicorn", "pillow"],
    port=3003,
...
placeholder(
    "photocha", "0.1.0", "Photocha", "Placeholder - no upstream binary available"
)

# photoshow - Placeholder
placeholder(
    "photoshow", "0.1.0", "Photoshow", "Placeholder - no upstream binary available"
)
...
python_git(
    ...
    8000,
    "/health",
    '["python3", "/opt/gallery3/manage.py", "runserver", "0.0.0.0:8000"]',
)
...
placeholder(
    "freenas", "0.1.0", "FreeNAS", "Placeholder - OS image, not containerizable"
)
...
placeholder(
    ...
    "0.1.0",
    "OpenMediaVault",
    "Placeholder - OS image, not containerizable",
)
...
java_app(
    ...
    4040,
    jar_name="airsonic-advanced-11.1.3.war",
...
java_app(
    ...
    4040,
    jar_name="airsonic-advanced-11.1.5.war",
    source_url="https://github.com/airsonic-advanced/airsonic-advanced/releases",
)

# tuneshell - Placeholder
placeholder(
    "tuneshell", "0.1.0", "TuneShell", "Placeholder - no upstream binary available"
)
...
python_git(
    ...
    6768,
    "/health",
    '["python3", "/opt/bazarr-subliminal/bazarr.py"]',
...
write_checksums(
    "qbittorrent-nox",
    "5.0.3",
    "https://github.com/qbittorrent/qBittorrent/archive/refs/tags/v5.0.3.tar.gz",
)
...
placeholder(
    "ollama-cuda", "0.5.7", "Ollama", "GPU placeholder - requires NVIDIA CUDA runtime"
)

# ollama-rocm - placeholder (GPU)
placeholder(
    "ollama-rocm", "0.5.7", "Ollama", "GPU placeholder - requires AMD ROCm runtime"
)
...
placeholder(
    ...
    "2.30.0",
    "LocalAI",
    "GPU placeholder - requires NVIDIA CUDA runtime",
)
...
placeholder(
    "embeddings", "0.1.0", "Embeddings", "Placeholder - no upstream binary available"
)
...
placeholder(
    "vllm-cuda", "0.6.6", "vLLM", "GPU placeholder - requires NVIDIA CUDA runtime"
)

# ai-engine - Placeholder
placeholder(
    "ai-engine", "0.1.0", "AI-Engine", "Placeholder - no upstream binary available"
)
...
placeholder(
    "qdrant-gpu", "1.17.1", "Qdrant", "GPU placeholder - requires NVIDIA CUDA runtime"
)
...
reference(
    "milvus-etcd", "3.5.17", "Milvus", "etcd", "Milvus etcd sidecar - delegates to etcd"
)
...
reference(
    ...
    "Milvus",
    "minio/minio",
    "Milvus minio sidecar - delegates to minio",
)
...
python_pip(
    ...
    ["chromadb"],
    port=8000,
    healthcheck_url="/api/v1/heartbeat",
...
reference(
    ...
    "Redis",
    "redis",
    "Redis with vector search - delegates to redis",
)
...
placeholder(
    "pytorch-gpu", "2.6.0", "PyTorch", "GPU placeholder - requires NVIDIA CUDA runtime"
)
...
placeholder(
    ...
    "2.6.0",
    "PyTorch",
    "CUDA placeholder - requires NVIDIA CUDA runtime",
)
...
python_pip(
    ...
    "7.3.0",
    "Jupyter",
    ["jupyterlab", "scikit-learn", "pandas", "numpy", "matplotlib"],
    port=8888,
    healthcheck_url="/api",
    entrypoint="jupyter-lab",
)
...
# retool - Placeholder
placeholder(
    "retool", "0.1.0", "Retool", "Placeholder - proprietary SaaS, no self-hosted binary"
)
...
java_app(
    ...
    8001,
    jar_name="airbyte-server-0.65.0.jar",
...
java_app(
    ...
    8002,
    jar_name="airbyte-worker-0.65.0.jar",
    source_url="https://github.com/airbytehq/airbyte/releases",
)
...
apt_get(
    "newsboat", "2.30", "Newsboat", ["newsboat"], user="newsboat", entrypoint="newsboat"
)

# newsblur - Placeholder
placeholder(
    "newsblur", "0.1.0", "NewsBlur", "Placeholder - no upstream binary available"
)
...
placeholder(
    "feediron", "0.1.0", "Feediron", "Placeholder - no upstream binary available"
)
...
write_checksums(
    "composer",
    "2.8.0",
    "https://github.com/composer/composer/releases/download/2.8.0/composer.phar",
)
