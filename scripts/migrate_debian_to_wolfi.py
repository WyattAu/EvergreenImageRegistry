...
class DockerfileMigrator:
    ...
    def migrate_dockerfile(self, df_path):
        ...
        from_lines = re.findall(r'^FROM\s+(.+?)(?:\s+AS\s+\w+)?\s*$', content, re.MULTILINE)
        if not from_lines:
            self.stats["skipped_no_debian"] += 1
            return

        last_from = from_lines[-1].strip()
        if not re.search(r'debian.*slim|debian:bookworm-slim|debian:bullseye-slim|debian:buster-slim', last_from, re.IGNORECASE):
            self.stats["skipped_no_debian"] += 1
            return
        ...
        if len(from_lines) > 1:
            ...
            debian_in_build = any(
                re.search(r'debian.*slim', f, re.IGNORECASE)
                for f in non_final
            )
    ...
    def determine_target(self, content, from_lines):
        ...
        apt_matches = re.findall(r'apt-get\s+install[^;\n]*?([a-zA-Z0-9][\w\-.=:]*)', final_stage)
        all_packages = []
    ...
    def apply_migration(self, content, from_lines, target, df_path):
        ...
        if is_multi:
            ...
            full_old_from = re.search(r'^FROM\s+' + re.escape(old_from_line.strip()) + r'\s*$', content, re.MULTILINE)
            if full_old_from:
                content = content[:full_old_from.start()] + new_from.rstrip('\n') + content[full_old_from.end():]
            else:
    ...
    def migrate_stage(self, from_line, stage_content, target, image_name, is_single=False):
        ...
        stage_content = re.sub(r'\bUSER\s+65534(?::\s*65534)?', 'USER 65532:65532', stage_content)
        stage_content = re.sub(r'\badduser\s+.*?--uid\s+65534', 'adduser -D -u 65532 nonroot', stage_content)
        stage_content = re.sub(r'\bgroupadd\s+.*?--gid\s+65534', 'addgroup -g 65532 nonroot', stage_content)
        stage_content = re.sub(r'\buseradd\s+.*?--uid\s+65534', 'adduser -D -u 65532 nonroot', stage_content)
        stage_content = re.sub(r'\busermod\s+.*?--uid\s+65534', 'usermod -u 65532 nonroot', stage_content)
        # Also update chown references
        stage_content = re.sub(r'--chown=65534:65534', '--chown=65532:65532', stage_content)
        stage_content = re.sub(r'chown\s+-R\s+65534:65534', 'chown -R 65532:65532', stage_content)
        ...
        if append_lines:
            ...
            stage_content = stage_content.rstrip('\n') + '\n' + '\n'.join(append_lines) + '\n'
    ...
    def _extract_packages_from_apt(self, apt_block):
        ...
        for token in re.split(r'\s+', pkg_str):
            ...
            if token and not token.startswith('#') and not token.startswith('-') and not token.startswith('$'):
                # Clean version pins
                ...
                if not token or len(token) < 2:
                    continue
                if token in ('apt-get', 'dpkg', 'apt', 'true', 'dev/null', 'bookworm',
                             'buster', 'bullseye', 'sid', 'testing', 'stable',
                             'auto-remove', 'no-install-recommends', 'yes'):
                    continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Migrate debian-slim Dockerfiles to wolfi/UBI")
