...
def expand_reference(
    reference: str, dockerfile: str, environment: dict | None = None
) -> str | None:
...
def _auth_header(challenge: str, timeout: float) -> str | None:
    ...
    request = urllib.request.Request(
        token_url, headers={"User-Agent": "evergreen-critical-digest-resolver/1"}
    )
    try:
...
def inspect_manifest(
    reference: str, timeout: float = 15.0, credentials: dict | None = None
) -> dict:
    ...
    username, password = (
        credentials.get(registry + "_USERNAME"),
        credentials.get(registry + "_PASSWORD"),
    )
    ...
    for attempt in range(2):
        try:
            request = urllib.request.Request(
                _manifest_url(registry, repository, tag), headers=headers
            )
            with urllib.request.urlopen(request, timeout=timeout) as response:
                body = response.read()
...
def _arg_values(
    dockerfile: str, environment: dict | None = None
) -> dict[str, set[str]]:
...
def propose_patch(
    ...
    original: str,
    digest: str,
    expanded_reference: str | None = None,
...
def eligible_for_index_pin(
    reference: str, media_type: str, is_index: bool, platforms: list[str]
) -> bool:
...
def generate_proposals(worklist: dict, resolutions: dict) -> list[dict]:
    ...
    by_key = {
        (item.get("image"), item.get("line")): item for item in resolutions["entries"]
    }
    proposals = []
    for entry in worklist["entries"]:
        ...
        if result.get("status") != "resolved":
            continue
        if not eligible_for_index_pin(
            result.get("reference", ""),
            result.get("media_type", ""),
            result.get("is_index", False),
            result.get("platforms", []),
        ):
            continue
        dockerfile = Path(entry["dockerfile"])
        text = dockerfile.read_text(errors="replace")
        replacement = propose_patch(
            text,
            entry["line"],
            entry["reference"],
            result["digest"],
            expanded_reference=result.get("expanded_reference"),
            ...
            proposals.append(
                {
                    ...
                    "line": entry["line"],
                    "old": text.splitlines()[entry["line"] - 1],
                    "new": replacement,
                    "digest": result["digest"],
                    "policy": "multi-platform-index",
...
def resolve_worklist(path: Path) -> dict:
    ...
    for entry in worklist["entries"]:
        ...
        dockerfile = (
            dockerfile_path.read_text(errors="replace")
            if dockerfile_path.exists()
            else ""
        )
        expanded = expand_reference(entry["reference"], dockerfile, os.environ)
        result = (
            inspect_manifest(expanded, credentials=os.environ)
            if expanded
            else {
                "reference": entry["reference"],
                "status": "unresolved-build-argument",
            }
        )
        result.update(
            {
        ...
        if result.get("status") == "resolved":
            ...
            result["proposal_status"] = (
                "ready" if result["proposal"] else "refused-ambiguous-target"
            )
        results.append(result)
    resolutions = {
        "schema_version": 4,
        "source_worklist_schema": worklist["schema_version"],
        "entries": results,
        "resolved": sum(item["status"] == "resolved" for item in results),
        "proposals_ready": 0,
        "unresolved": sum(item["status"] != "resolved" for item in results),
    }
...
def main() -> None:
    ...
    print(
        f"Resolved {result['resolved']}/{result['resolved'] + result['unresolved']} references; proposals ready: {result['proposals_ready']}"
    )
