...
**Resolution Documented:** [ADR-005](.adrs/ADR-005-military-compliance-framework.md) **Implemented In:**
[REQUIREMENTS.md](.specs/00_requirements/requirements.md) Part II §2.1 (C001, C007)
...
**Resolution Documented:** [ADR-005](.adrs/ADR-005-military-compliance-framework.md) **Implemented In:**
[REQUIREMENTS.md](.specs/00_requirements/requirements.md) Part VIII §8.1 (Compliance Framework)
...
**Resolution Documented:** [ADR-005](.adrs/ADR-005-military-compliance-framework.md) **Implemented In:**
[REQUIREMENTS.md](.specs/00_requirements/requirements.md) Part I §1.1 (Preference Order), Part IV §4.2 (Size Limits)
...
**Implemented In:** [REQUIREMENTS.md](.specs/00_requirements/requirements.md) Part IV §4.2
...
**Resolution:** **Eliminated tier-based mapping entirely.** See ADR-007 — universal preference order applies to all
tiers: scratch > wolfi > UBI micro > UBI minimal > UBI standard.

**Implemented In:** [REQUIREMENTS.md](.specs/00_requirements/requirements.md) Part I §1.1,
[ADR-007](.adrs/ADR-007-base-image-preference-order.md)
...
**Resolution:** **Replaced Docker HEALTHCHECK instruction with HTTP health probes.** All images serve /livez, /readyz,
/startupz on port 9101. See ADR-006.

**Implemented In:** [REQUIREMENTS.md](.specs/00_requirements/requirements.md) Part III §3.1,
[ADR-006](.adrs/ADR-006-observability-architecture.md)
...
**Resolution:** No init system **baked into the image** (C023). Runtime injects init via `docker run --init` or K8s
`shareProcessNamespace: true`. The image never contains tini/dumb-init/systemd.

**Implemented In:** [REQUIREMENTS.md](.specs/00_requirements/requirements.md) Part II §2.2 (C023)
...
**Resolution:** All three conflicts are resolved in ADR-005 (Military Compliance Framework). Updated references in this
document.
...
**Implemented In:** [REQUIREMENTS.md](.specs/00_requirements/requirements.md) Part I §1.2 (Ban), Part II §2.1 (C003
note)
...
**Same-Priority Resolution:** Document both requirements → Analyze impact → Apply most restrictive → Create ADR → Update
compliance matrix.
