...
def extract_binary(df: Path) -> str | None:
    ...
    for line in content.split("\n"):
        ...
        if s.startswith("ENTRYPOINT [") or s.startswith("CMD ["):
            m = re.search(r'\["([^"]+)"', s)
            if m and m.group(1) not in ("/bin/sh","/bin/bash","/busybox/sh","sh","bash"):
                return m.group(1)
        if s.startswith("ENTRYPOINT ") and not s.startswith("ENTRYPOINT ["):
            p = s.split()
            if len(p)>1 and p[1].strip('"') not in ("/bin/sh","/bin/bash","sh","bash"):
                return p[1].strip('"')
    ...
    for line in df.read_text().split("\n"):
        if line.strip().startswith("EXPOSE "):
            for n in re.findall(r'\b(\d+)\b',line):
                if n != "9101": return int(n)
    return 8080

CAT_MAP = {
    "database": ["sql","db","mongo","redis","postgres","mysql","mariadb","cassandra",
                 "couch","etcd","neo4j","influx","timescale","valkey","memcached",
                 "arangodb","clickhouse","cockroach","duckdb","elasticsearch",
                 "opensearch","meilisearch","questdb","scylla","crate"],
    "monitoring": ["prometheus","grafana","alert","metric","monitor","loki","tempo",
                   "mimir","thanos","exporter","telegraf","netdata","zabbix","uptime"],
    "security": ["vault","cert","scan","trivy","grype","falco","audit","firewall",
                 "wazuh","suricata","clamav","authelia","authentik","keycloak",
                 "openldap","freeipa","rkhunter","chkrootkit"],
    "proxy": ["nginx","traefik","haproxy","envoy","caddy","proxy","gateway",
              "ingress","apache","squid"],
    "messaging": ["kafka","rabbit","nats","pulsar","mqtt","zeromq","activemq",
                  "emqx","vernemq","rocketmq"],
}
...
def main():
    ...
    for d in sorted(IMAGES_DIR.iterdir()):
        if not d.is_dir() or d.name in ("tests","health-shim"): continue
        df = d / "Dockerfile"
    ...
    print(f"test_config.yaml updated")
