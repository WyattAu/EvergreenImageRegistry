...
**Date:** 2026-06-14 **Auditor:** Nexus (automated) **Method:** Dockerfile analysis for SIS-critical images
...
**Cause:** Dockerfile template generates ENTRYPOINT with `/shim` (correct for scratch) but wolfi-based images copy shim
to `/usr/local/bin/shim`. The HEALTHCHECK line correctly uses `/usr/local/bin/shim`, proving the file knows the right
path — ENTRYPOINT was not updated to match.

**Systemic Fix:** Audit all wolfi-base images with `COPY --from=shim /shim /usr/local/bin/shim` and verify ENTRYPOINT
matches.

### Pattern 2: Missing App Code

**Cause:** Images generated from templates that set up the runtime environment (PHP, Node, Python) but never COPY the
actual application from upstream. This matches the "pkg-install" build type pattern where the generator installs
packages but doesn't fetch the app.
