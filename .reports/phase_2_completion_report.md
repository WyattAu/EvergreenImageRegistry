...
Phase 2 implemented comprehensive runtime security controls for all 223 container
images. The primary deliverables include seccomp profiles for 5 workload categories,
AppArmor profiles for 4 workload categories, automated test scripts for both
profile types, documented hardening patterns for symbol stripping and static linking,
capabilities audit with `--cap-drop ALL`, and image size enforcement tiers in CI.
...
**Status:** COMPLETE
**Location:** `images/tests/profiles/`
...
- **Container testing:** Runs container with `--security-opt seccomp=<profile>`, `--cap-drop ALL`, `--security-opt no-new-privileges:true`
...
- **Container testing:** Runs with `--security-opt apparmor=<profile>`, `--cap-drop ALL`, `--security-opt no-new-privileges:true`
...
This removes all symbol table and relocation information from binaries,
reducing image size and attack surface (no debug symbols to aid reverse
engineering). Applied consistently in multi-stage build patterns across
all scratch and distroless image Dockerfiles.
...
All scratch-based images should contain only statically linked binaries.
This is verified as part of the build pipeline to ensure no shared library
dependencies leak into minimal images.
...
**Enforcement:** Post-build step in CI checks image size against tier limits
and fails the build if exceeded. This prevents image bloat from creeping in
over time.
...
**Classification: RUNTIME SECURITY**
