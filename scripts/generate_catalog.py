...
def categorize(name):
    ...
    for category, patterns in CATEGORY_PATTERNS.items():
        for pattern in patterns:
            if pattern.lower() == name_lower or name_lower.startswith(
                pattern.lower() + "-"
            ):
                return category
    return "other"
...
def scan_images():
    ...
    for entry in sorted(IMAGES_DIR.iterdir()):
        ...
        base_image = meta["base_image"] or manifest.get(
            "base_image", manifest.get("runtime_image", "unknown")
        )
        ...
        hardened = (
            meta["labels"].get("evergreen.constraint.hardened", "false") == "true"
        )
